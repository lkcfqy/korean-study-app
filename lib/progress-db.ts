...
import {lessons,type LessonProgress,type Progress} from './course';
...
export async function readProgress(userId:string):Promise<Progress>{
 ...
 return {rows:result.results,currentLesson:result.results[0]?.lesson_id??lessons[0].id,serverTime:Date.now()};
}
