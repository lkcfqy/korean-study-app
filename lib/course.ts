...
export const lessons:LessonMeta[]=index.lessons.map(row=>{
 const [id,title,day,lineCount,revision]=row as [string,string,number,number,string];
 return {id,title,day,lineCount,revision,stage:Math.floor((day-1)/30)};
});
...
export type Progress={rows:LessonProgress[];currentLesson:string;serverTime:number;counts:{sentences:number;words:number;completed:number}};
const countsSchema=z.object({sentences:z.number().int().nonnegative(),words:z.number().int().nonnegative(),completed:z.number().int().nonnegative()});
export const progressSchema=z.object({rows:z.array(z.object({lesson_id:z.string(),cursor:z.number().int().nonnegative(),read_mask:z.number().int().nonnegative(),completed_at:z.number().nullable(),next_review_at:z.number().nullable(),review_step:z.number().int(),updated_at:z.number(),draft:z.string(),draft_revision:z.number().int()})),currentLesson:z.string(),serverTime:z.number(),counts:countsSchema});
