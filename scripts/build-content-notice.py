...
<p>此前发布前做过音频解码、静音和削波检查，完整句子还经过不提供原文提示的自动转写复核，低匹配项单独复核。随后注释改进中新增的原形、独立词块和一条更正拼写的句子，有单独的信号检查，但未计入该基准转写报告。随后补齐了全部可点击词块的完整词形音频；这些新增片段有解码、信号、文件哈希和引用检查，未计入旧的句子自动转写覆盖。既有完整句子和原形音频保持不变。自动转写与信号检查不能代替母语者逐条听审，尚未完成全部音频的人工听审。</p>
<h2>署名与许可</h2>
