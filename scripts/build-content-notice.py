...
contrast_count = len(json.loads((ROOT/'content/contrast-lessons.json').read_text()))
...
personal_rows = [json.loads(line) for line in (ROOT/'docs/independent-review-20260923.jsonl').read_text().splitlines() if line.strip()]
if len({row['lessonId'] for row in personal_rows}) != len(personal_rows):
    raise SystemExit('Duplicate independent review lesson')
for row in personal_rows:
    lesson_bytes = (ROOT/'public/course'/f'{row["lessonId"]}.json').read_bytes()
    if hashlib.sha256(lesson_bytes).hexdigest() != row['sha256']:
        raise SystemExit(f'Independent review is stale for {row["lessonId"]}')
personal_lessons = len(personal_rows)
...
source.update({
    ...
    'courseModifications': 'Selection and ordering; Chinese sentence translation and AI and direct editorial correction; morphological annotations; 180-day allocation; comprehension questions.',
...
<p class="numbers">{len(course):,} 关 · {index['totalWords']:,} 个去重词条 · {index['totalSentences']:,} 条去重对话<br>按自己的节奏学习 · 180 天为高强度参考安排</p>
<h2>这些数字对应什么</h2>
...
<p>此前的定向复评修正或澄清了 128 处注释，包括 하다 的动作与点餐选择义、하나 的数词义、별로 的否定语境，게（것 + 이）的名词化用法，以及 아냐 的否定判断、살 的体重语境和复合数词。每处保留编辑决定与词典义项。这是 AI 编辑核对，仍非独立教师认证。</p>
...
<p>2026-09-23 开始由 Codex 在本次任务中直接逐课读取韩语原文、中文译文、全部可点词块、课程词义及两道题，并以课程文件 SHA-256 记录复核结果。当前版本中，已完成 {personal_lessons:,}/{len(course):,} 关；剩余 {len(course)-personal_lessons:,} 关尚未完成这一级别的独立逐条审校。本轮没有调用本地语言模型。已审关卡发现并修正了语境词义、语法说明和译文问题；全库审校完成前不能据此推断其余关卡正确。</p>
<h2>180 天如何安排</h2>
...
<h2>听力检查与巩固</h2>
<p>每关练习两句。{contrast_count:,} 关使用逐题编写的听力对比选项，完整听完后才能作答；其他 {recall_count:,} 关改为先听、用自己的话回忆，再展开韩中对照核对，并选择“主要意思一致”或“没想起或有遗漏”。自评只用于安排复习，不显示为客观听力得分。使用韩文提示、未听完或有遗漏的内容进入短间隔巩固；两种练习都不能证明已经掌握或能迁移到陌生对话。</p>
<p>登录后可以选择每组 4、8 或 12 关，系统优先安排待巩固和到期复习，空余位置最多加入 4 关新课；积压较多时本组不加入新课。分组完成数只在完成本关练习后增加；读完对话、打开关卡不算完成。分组是当次会话安排，刷新可以重新分组，各关学习进度仍在云端。</p>

<h2>固定 SunHi 读音</h2>
<p>音频库保留 {len(audio['entries']):,} 段固定的 ko-KR-SunHiNeural 合成音频，涵盖句子、句中完整词形、词典原形及拼读示例，并保留旧版兼容文件，正常语速生成；慢速播放保留音高。句子整段合成，保留连读和语调，不拼接单词音频。</p>
<p>每个可点击词块都提供“听句中词形”，包含实际出现的助词、时态和词尾，例如 약을、먹으니、주차했어요；词典原形在单独的“听原形”选项中播放。词形片段主要按完整书写形式单独合成，个别短词使用已记录的原句片段；跨词连读和整句语调请使用“听整句”。</p>
<p>全部 {len(audio['entries']):,} 段音频已完成文件引用、SHA-256 与解码/信号检查，并纳入不提供原文提示的盲转写扫描；包含句子、独立词形、原形和拼读示例。分组转写存在短词边界误差，差异项另做单独转写和严格的拼写到读音比对；转写不一致是复核线索，不能直接认定语音错误。更换韩文文本时同步准备对应的句子、词形与原形音频。第二识别引擎已复核 {second["currentHashBoundSecondEngineReviews"]:,} 段有转写差异的音频，其中 {second["classification"]["independent_transcript_match"]:,} 段增加文本一致证据，{second["classification"]["difference_persists"]:,} 段仍有差异。自动检查不能代替母语者逐条听审，尚未完成全部音频的人工听审。</p>
<p>2026-09-23 对全部课程播放引用再次核对，未发现文本和音频文件错配。仍有 5 组双引擎共同疑点优先待母语者试听；在听审确认前，不把识别差异写作错读，也不凭清单声线标签认证每段实际发音。</p>
<h2>署名与许可</h2>
...
<small>课程与审校说明更新：2026-09-23</small></main></html>'''
