...
<h2>听力检查与巩固</h2>
<p>每关练习两句。38 关使用逐题编写的听力对比选项，完整听完后才能作答；其他 3,968 关改为先听、用自己的话回忆，再展开韩中对照核对，并选择“主要意思一致”或“没想起或有遗漏”。自评只用于安排复习，不显示为客观听力得分。使用韩文提示、未听完或有遗漏的内容进入短间隔巩固；两种练习都不能证明已经掌握或能迁移到陌生对话。</p>
...
<h2>固定 SunHi 读音</h2>
<p>句子、句中完整词形、词典原形和拼读示例使用 {len(audio['entries']):,} 段固定的 ko-KR-SunHiNeural 合成音频，正常语速生成；慢速播放保留音高。句子整段合成，保留连读和语调，不拼接单词音频。</p>
...
<p>全部 {len(audio['entries']):,} 段音频已完成文件引用、SHA-256 与解码/信号检查，并纳入不提供原文提示的盲转写扫描；包含句子、独立词形、原形和拼读示例。分组转写存在短词边界误差，差异项另做单独转写和严格的拼写到读音比对；转写不一致是复核线索，不能直接认定语音错误。此次释义修正补齐 아니다 的原形音频，原有 28,777 段音频字节保持不变。自动检查不能代替母语者逐条听审，尚未完成全部音频的人工听审。</p>
<h2>署名与许可</h2>
