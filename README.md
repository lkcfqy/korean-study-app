...
- 102 关提供 204 道专门编写的听力对比题；其余 3,904 关使用先回忆、再核对、自评的练习。自评与首次独立作答分开显示；未听完、使用提示或自评有遗漏会进入短间隔巩固。答案位置保留以兼容旧客户端。
...
全部 28,779 段音频已完成文件引用、SHA-256 与解码/信号检查，并纳入不提供原文提示的盲转写扫描；包含句子、独立词形、原形和拼读示例。分组转写存在短词边界误差，差异项另做单独片段、重复片段转写和严格的拼写到读音比对；这些复核使用同一个模型，转写不一致不能直接认定语音错误。继上一轮补齐 아니다 原形音频后，本轮补齐 옴，原有 28,778 段音频字节保持不变。当前仍有 1,731 段转写差异（485 段对话、1,246 段词形等），没有把未确证项记为听审通过。自动检查不能代替母语者逐条听审。全量扫描与差异分类见 `docs/audio-inventory-review.json`。
