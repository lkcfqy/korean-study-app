...
- 28,778 段固定 ko-KR-SunHiNeural 音频（含保留的旧文本音频）；其中 13,660 段为本次新增的完整词形读音，覆盖全库 16,374 个不同词形，其余词形复用已有音频。句子、词条、拼读示例都有音频。提供保持音高的 0.8 倍速。
...
- 38 关保留专门编写的听力对比题；其余 3,968 关使用先回忆、再核对、自评的练习。自评与首次独立作答分开显示；未听完、使用提示或自评有遗漏会进入短间隔巩固。答案位置保留以兼容旧客户端。
...
全部 28,778 段音频已完成文件引用、SHA-256 与解码/信号检查，并纳入不提供原文提示的盲转写扫描；包含句子、独立词形、原形和拼读示例。分组转写存在短词边界误差，差异项另做单独转写和严格的拼写到读音比对；转写不一致是复核线索，不能直接认定语音错误。此次释义修正补齐 아니다 的原形音频，原有 28,777 段音频字节保持不变。自动检查不能代替母语者逐条听审，尚未完成全部音频的人工听审。 全量扫描与差异分类见 `docs/audio-inventory-review.json`；没有把所有自动转写差异当成发音错误或已解决。

2026-09-21 复评改进又确认或澄清 128 处注释，覆盖同音词、否定、体重搭配、选择义与复合数词；新增的整句中文校正同样有记录。详见 `docs/editorial-corrections-20260921.json`。模型提出的错误建议经人工式逐项比较后予以拒绝，未直接批量采纳。
...
- `docs/progress-tests.json`：28 项本地生产 Worker 与 D1 的账号隔离、续学、首次答错/提示、短间隔巩固、并发、版本冲突及完整学习历史统计验证。
