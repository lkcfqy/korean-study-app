...
- 28,779 段固定 ko-KR-SunHiNeural 音频（含保留的旧文本音频）；其中 13,660 段为完整词形专项新增读音，覆盖全库 16,374 个不同词形，其余词形复用已有音频。句子、词条、拼读示例都有音频。提供保持音高的 0.8 倍速。
...
| --- | --- | ---: | ---: |
| 1 | 拼读、基础句式与生活表达 | 897 | 792 |
| 2 | 熟悉日常对话，稳固 TOPIK I 基础 | 2,161 | 2,244 |
| 3 | 经历、理由与长句，进入 TOPIK II | 3,125 | 3,807 |
| 4 | 社会情境、信息核实与概括 | 4,159 | 5,187 |
| 5 | 抽象观点、让步与论证 | 5,216 | 6,854 |
...
全部 28,779 段音频已完成文件引用、SHA-256 与解码/信号检查，并纳入不提供原文提示的盲转写扫描；包含句子、独立词形、原形和拼读示例。分组转写存在短词边界误差，差异项另做单独片段、重复片段转写和严格的拼写到读音比对；这些复核使用同一个模型，转写不一致不能直接认定语音错误。继上一轮补齐 아니다 原形音频后，本轮补齐 옴，原有 28,778 段音频字节保持不变。当前仍有 1,731 段转写差异（485 段对话、1,246 段词形等），没有把未确证项记为听审通过。自动检查不能代替母语者逐条听审。全量扫描与差异分类见 `docs/audio-inventory-review.json`。随后又用独立的 SenseVoiceSmall 在本机盲识别全部 1,731 段差异音频：388 段文字相符、1,343 段差异持续。原 Whisper 分类保留；没有修改音频，也不把交叉一致当作发音认证，详见 `docs/audio-second-engine-review.json`。

2026-09-21 上一轮复评确认或澄清 128 处注释，见 `docs/editorial-corrections-20260921.json`。本轮继续逐项复核 216 个待处理关卡，新增 90 处词块修正或澄清和 11 处整句中文调整；通用语法和词义规则再生成后共改变 563 关的 635 个词块，不能将生成差异与显式编辑数量相加。新增 64 关、128 道同场景听力对比题。来源与范围见 `docs/editorial-corrections-extreme-20260921.json`、`docs/extreme-review-summary.json` 和 `docs/listening-contrasts-extreme-20260921.json`。模型提出的错误建议经 Codex AI 编辑逐项比较后予以拒绝，未直接批量采纳；不是独立教师认证。最新评分与具体边界见 [92 分复评](docs/quality-result.md)。
