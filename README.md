...
- 一句一学、整句可点击拆解（216 句 / 1,051 个词块）、词典原形音频、两句理解检查和到期对话复习。所有关卡自由进入，无前置关卡要求。
- 登录账号下的对话位置、学过词句、完成关卡、复习时间与高级关卡书面回复保存在 D1。
- 不使用 localStorage 作为学习进度来源。服务端保存失败时不推进学习步骤，保留当前内容以便重试。

站点已向所有人开放；访客可自由试学，访客试学不保存进度。登录后的进度按账号隔离。

**180 天学习安排已完成，站内精读内容为 216 个独立句。** 日程以现有对话作精读锚点，配合同主题官方教材和公开题扩展。6,000 词条 / 8,000 句是含站外材料的输入目标，不是已经收录的数量。本项目不会把闯关、词句接触次数当成 TOPIK 等级认证。

## 半年课程的设计目标

按每天 4 小时、共 720 小时规划，180 天均有具体任务、时间分配、建议新词句配额和对应精读对话。每 5 天安排一天复习，第 30、60、90、120、150、180 天安排阶段检查，第 170 天增加一次完整检验。完整日程见 [180 天计划](docs/180-day-plan.md)。累计输入目标为 6,000 个核心词条和 8,000 个独立句。该数量是项目设计目标，不是 TOPIK 官方词汇门槛，也不代表现在已经收录。
...
韩中对话为原创内容。本轮逐句复核 216 条译文并编写、检查 1,051 个词块说明，覆盖整句中的词义、助词、时态、敬语与语尾；修正了 4 处需要更准确处理语境的译文。所有拆解表层文字与原句逐字符一致，未使用自动分词后随意拼接词义。

729 段音频全部可解码为 PCM，未发现静音文件或削波采样；完整句子音频保持原样，拆解不把语尾片段拼接成朗读。独立双语审校、逐条人工听审、手机真机及线上两设备验收尚未完成，不能将技术检查称作翻译或发音的绝对保证。

- `content/course.json` 是实际发布的课程源。
- `content/breakdowns.txt` 是逐句编写的拆解源，`scripts/compile-breakdowns.py` 检查并合并到课程。
- `content/study-plan.json` 是完整 180 天日程，由 `scripts/build-study-plan.py` 生成，UI 与 Markdown 使用同一数据。
- `content/audio-manifest.json` 记录合成音色、文本、路径和文件大小。
- `docs/content-audit.json` 记录内容检查范围。
- `docs/audio-signal-tests.json` 记录全部音频的 PCM 解码与信号检查。
- `docs/progress-tests.json` 记录本地接口验证结果；不代表线上端到端验证。
- `docs/browser-qa.json` 记录访客自由跳关、词块拆解、音频播放启动、手机尺寸日程和只读进度工具验证。
...
python3 scripts/compile-breakdowns.py
python3 scripts/build-study-plan.py
python3 scripts/validate-course.py
python3 scripts/check-audio-signal.py
```
