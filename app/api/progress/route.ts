...
import {z} from 'zod';
import {listeningCheckSchema,independentAnswers} from '../../../lib/listening-check';
...
const bodySchema=z.discriminatedUnion('action',[
 ...
 z.object({action:z.literal('complete'),lessonId:z.string(),answers:z.array(z.number().int().min(0).max(2)).length(2),check:listeningCheckSchema.optional()}),
...
export async function POST(request:Request){
 ...
 try{
  ...
  if(body.action==='select'){
   ...
   const outcome=body.check?(independentAnswers(body.check,lesson.answers)===2?1:0):-1;
   await db.prepare(`UPDATE lesson_progress SET
    ...
    .bind(now,lesson.lineCount,now,now+86400000,outcome,now,now+86400000,now+86400000,outcome,now,now+86400000,now+86400000,now,now,outcome,outcome,now,uid,lesson.id).run();
