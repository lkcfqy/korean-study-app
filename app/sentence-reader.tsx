...
export default function SentenceReader({line,meaning,onListen,onListenWord}:{line:Line;meaning:boolean;onListen:()=>void;onListenWord:(path:string)=>void}){
 ...
 const readings=part?.readings?.filter(word=>word.audio!==part.surfaceReading.audio)??[];
 return <>
  ...
  {part&&<div id={noteId} className="selected-part" role="region" aria-label="选中词语的注释" aria-live="polite" onKeyDown={event=>{if(event.key==='Escape'){if(selected!==null)tokens.current[selected]?.focus();setSelected(null);event.stopPropagation();}}}><div className="overview-row"><strong lang="ko">{part.text}</strong><span>{part.meaning}</span></div><p>{part.explanation}</p><div className="context-pronunciation"><button className="button primary surface-pronunciation" aria-label={'听句中词形 '+part.surfaceReading.term} onClick={()=>onListenWord(part.surfaceReading.audio)}><Volume2 size={16}/>听句中词形 <span lang="ko">{part.surfaceReading.term}</span></button>{readings.map(word=><button key={word.term} className="text-link" aria-label={'听原形 '+word.term} onClick={()=>onListenWord(word.audio)}><Volume2 size={16}/>听原形 <span lang="ko">{word.term}</span></button>)}<button className="text-link" onClick={onListen}><Volume2 size={16}/>听整句</button></div></div>}
 </>;
}
